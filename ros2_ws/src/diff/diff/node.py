...
DEFAULT_MAX_SPEED = 0.25     # max |cmd| (0–1) from cmd_vel mapping
...
# Safety / control
DEFAULT_CMD_TIMEOUT_S  = 0.5   # stop if no cmd_vel in this time
DEFAULT_ODOM_RATE_HZ   = 50.0  # odom update rate
...
class Odometry:
    ...
    Convention (REP-103 / ROS standard):
        +x  = forward
        +y  = left
        +θ  = counter-clockwise (CCW)
    """
...
class DiffDriveNode(Node):
    def __init__(self):
        ...
        self.odom = Odometry(dist_per_tick=dist_per_tick,
        ...
        self.odom_timer = self.create_timer(1.0 / self.odom_rate_hz,
                                            self.update_loop)
    ...
    def update_loop(self):
        ...
        # Convert cmd_vel -> wheel commands
        v = self.current_cmd.linear.x        # m/s
        w = self.current_cmd.angular.z       # rad/s

        # Differential drive inverse kinematics [web:25][web:35]
        v_r = v + (w * self.wheel_base / 2.0)
        v_l = v - (w * self.wheel_base / 2.0)

        # Max wheel linear speed mapped to |cmd| <= max_speed_cmd
        max_wheel_speed = self.max_speed_cmd  # interpret as "1.0 == some max m/s"
        # Simple normalization: assume 1.0 cmd ~ max_wheel_speed m/s
        cmd_r = clamp(v_r / max_wheel_speed, -1.0, 1.0)
        cmd_l = clamp(v_l / max_wheel_speed, -1.0, 1.0)

        # Apply deadband and send to motors
    ...
    def publish_odom_and_tf(self, stamp, x, y, theta):
        ...
        # Rough velocity estimate from current cmd_vel (you can also compute from delta pose)
